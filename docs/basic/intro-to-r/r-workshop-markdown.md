...
- By increasing the number of hashtags you decrease the size of the header
- Lists can be specified by adding a dash or asterisk before the text
- for more information on Mark down formatting visit:
  https://www.markdownguide.org/basic-syntax/
  
## Code chunks
  
- code chunks can be included in our markdown document with two sets of three tick marks
- you'll notice in the brackets:
- r: the type of code
- code: the name of the code chunk
- warning=F: an option not to include any warnings
- message=F; an option not to include any messages 
...
- after pointing to this location we can import a pacakage!
- here we imported the tidyverse package using the library() function

## Importing Data

So what does this pacakage do? 
...
```{r}
# load our data
meta <- read_delim(file="../data/meta.tsv",delim = "\t")
```
...
- we specified the delimeter or the separator between our data
...
NOTE: If you want more information on R data types visit:
...
- Let's try to use these operators to ask two questions in our data:
...
- For a full list of R markdorm output options visit:
