...
When importing .csv files you'll need to specify the path to where you're file is located. So if your .csv file is in `/new-project/test.csv`, you can download it like so:

```
test <- read.csv("/new-project/test.csv")
```
